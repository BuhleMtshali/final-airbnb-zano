...
export const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Replace later with actual user ID once your backend is hooked up
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            {mockReservations.slice(0, 2).map((reservation) => (
              <div key={reservation.id} className="flex items-center space-x-4">
                <div className="flex-shrink-0 h-16 w-16 rounded-md overflow-hidden">
                  <img src={reservation.image} alt={reservation.listingTitle} className="h-full w-full object-cover" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-900 truncate">{reservation.listingTitle}</p>
                  <p className="text-sm text-gray-500 truncate">{reservation.location}</p>
                </div>
                <div className="text-right">
      ...
      <div className="bg-white shadow rounded-lg p-6 md:col-span-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Link to="/create-listing" className="flex flex-col items-center p-4 border border-gray-200 rounded-lg hover:bg-gray-50">
          ...
          <Link to="/my-listings" className="flex flex-col items-center p-4 border border-gray-200 rounded-lg hover:bg-gray-50">
            <ListIcon className="h-8 w-8 text-[#FF5A5F] mb-2" />
            <span className="text-sm font-medium text-gray-900">Manage Listings</span>
          </Link>
          <Link to="/reservations" className="flex flex-col items-center p-4 border border-gray-200 rounded-lg hover:bg-gray-50">
                ...
                <div className="mt-4 flex justify-between items-end">
                  ...
                  <div className="flex space-x-2">
                    ...
                    <button
                      ...
                      className="inline-flex items-center px-3 py-1.5 border border-transparent shadow-sm text-sm font-medium rounded text-white bg-[#FF5A5F] hover:bg-[#FF4A4F]"
                    >
                      Cancel
                    </button>
                  </div>
  ...
  return (
    <div className="bg-gray-100 min-h-screen py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {activeTab === 'listings' && (
          <div className="bg-white p-6 rounded-lg shadow">Listings Tab Placeholder</div>
        )}
